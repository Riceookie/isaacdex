...
import { nasluchujGlosu } from '@/lib/companionGlos'
...
export default function CompanionMascot({ steamConnected = true }: { steamConnected?: boolean }) {
  ...
  // Kwestia „na żądanie" z innej części apki (np. czat: wysłałeś wiadomość, wszedłeś na kanał).
  // Trzymana w refie + tick, bo ma PRZERWAĆ trwającą pętlę, a nie czekać na jej koniec.
  const wtret = useRef<string | null>(null)
  const [wtretTick, setWtretTick] = useState(0)

  useEffect(
    () =>
      nasluchujGlosu((tekst) => {
        wtret.current = tekst
        setWtretTick((t) => t + 1)
      }),
    [],
  ...
  useEffect(() => {
    ...
    async function pisz(text: string) {
      setTalking(true)
    ...
    async function run() {
      // Wtręt z czatu wchodzi natychmiast, przed kwestiami z puli.
      if (wtret.current) {
        const text = wtret.current
        wtret.current = null
        await pisz(text)
        if (!cancelled) await sleep(5000)
        if (!cancelled) await wymaz(text)
  ...
  }, [pathname, steamConnected, muted, wtretTick])
